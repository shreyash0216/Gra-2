...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('gra_plans');
    if (saved) {
      try {
  ...
  const handleFormSubmit = async (data: VillageData) => {
    ...
    try {
      ...
      setAdaptationPlan(result);
      setSavedPlans((prev: AdaptationPlan[]) => [result, ...prev]);
  ...
  const handleDeletePlan = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    setSavedPlans((prev: AdaptationPlan[]) => prev.filter((p: AdaptationPlan) => p.id !== id));
  };
              ...
              {savedPlans.map((p) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-500">{p.recommendations.length} Recommendations</span>
      ...
      <footer className="bg-navy-950 text-white/60 py-20 px-4 mt-20">
        <div className="container mx-auto max-w-5xl text-center space-y-8">
          ...
          <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-sm">
            <p>© 2025 GRA. Powered by ML & Gemini RAG.</p>
